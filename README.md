# MyBosco — Система управління навчальним процесом

**MyBosco** — веб-платформа для автоматизації академічного обліку, побудована на **Django 5.2** та **MySQL**. Охоплює повний цикл навчального процесу: від розкладу та журналу оцінок до домашніх завдань, новинної стрічки та SMS-сповіщень.
...
- Налаштування закладу та шкал оцінювання
...
- Перегляд та редагування конкретних занять у часовій шкалі
- Завантаження матеріалів і вкладень до занять
...
- Перегляд, подача та скасування домашніх завдань з файловими вкладеннями
- Розклад занять
- Просунуті фільтри: за датою, предметом, балом, пошуковим запитом

### Для всіх
- Новинна стрічка з постами та коментарями
- Система сповіщень (з позначками "прочитано/непрочитано")
- Перемикання теми інтерфейсу (світла / темна)
- SMS-сповіщення через Twilio при виставленні оцінки або пропуску
...
│   ├── templatetags/
│   │   ├── journal_filters.py   # Фільтри для журналу
│   │   └── math_filters.py      # Математичні обчислення у шаблонах
...
- Всі паролі хешуються через PBKDF2 (стандарт Django)
- Рольовий доступ (Admin / Teacher / Student) через декоратор `role_required`
- Django ORM захищає від SQL Injection
- Middleware `NoCacheAuthMiddleware` запобігає кешуванню сторінок авторизованих користувачів
